...
public interface ItemRepository extends JpaRepository<Item, Long>{
	...
	@Query("Select DISTINCT i.itemDescription FROM Item i WHERE i.itemCategory = ?1 AND i.itemMake = ?2")
	public List<String> getItemDescByCategoryMake(String itemCategory, String itemMake);
	
	@Query("Select DISTINCT i.itemValuation FROM Item i WHERE i.itemCategory = ?1 AND i.itemMake = ?2 AND i.itemDescription = ?3")
